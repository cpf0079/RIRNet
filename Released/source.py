...
class MyNet(nn.Module):
    def __init__(self, num_level=3, length=40, iter=3, input_dim=2048, reduced_dim=512, hidden_dim=512,
                 num_frames=40):
        ...
        self.p = nn.Sequential(
            nn.Linear(num_grid * self.reduced_dim, self.reduced_dim),
        ...
        self.conv_1 = nn.Sequential(
            nn.Conv2d(in_channels=self.input_dim, out_channels=self.reduced_dim, kernel_size=1,
                                stride=1, padding=0),
...
if __name__ == "__main__":
    ...
    parser.add_argument('label_path', type=str,
                        help='your training label path')
    parser.add_argument('frame_path', type=str,
                        help='your training frame path')
...
train_data = MyDataset(txt_dir=args.label_path,
                       root=args.frame_path,
                       transform=transform)
...
for epoch in range(args.epochs):
    ...
    for i, (images, labels) in enumerate(train_loader):
        ...
        loss = criterion(outputs[0], labels) \
               + 0.5 * (criterion(outputs[1], labels) + 0.3 * criterion(outputs[2], labels) + 0.2 * criterion(outputs[3], labels))
